...
export function rtiPage(): string {
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 flex items-start gap-3">
        ...
        <div class="text-sm text-amber-800">
          <strong>What is RTI?</strong> The Right to Information Act 2005 gives every Indian citizen the right to demand information from public authorities. If your CPGRAMS complaint has been ignored for 30+ days, an RTI forces the department to explain what action they took.
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-6">
        ...
        <div class="p-6 space-y-4">
          <div>
            ...
            <input type="text" id="rtiName" placeholder="Enter your full name" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-red-400">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">CPGRAMS Complaint ID</label>
            <input type="text" id="rtiCpgrams" placeholder="e.g., PMOPG/E/2026/0012345" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-red-400">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Date Complaint Was Filed</label>
            <input type="date" id="rtiDate" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-red-400">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Department/Ministry Name</label>
            <input type="text" id="rtiDept" placeholder="e.g., Ministry of Agriculture" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-red-400">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Brief Summary of Your Complaint</label>
            <textarea id="rtiSummary" rows="3" placeholder="Summarize the issue you originally complained about" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-red-400 resize-none"></textarea>
          </div>
          <button onclick="generateRTI()" id="generateBtn" class="w-full bg-gradient-to-r from-red-600 to-red-700 text-white py-3 rounded-xl font-semibold hover:from-red-700 hover:to-red-800 transition-all flex items-center justify-center gap-2">
      ...
      <div id="rtiPreview" class="hidden">
        <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-6">
          <div class="px-6 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
            ...
            <div class="flex gap-2">
              <button onclick="copyRTI()" class="text-xs text-saffron-600 hover:text-saffron-700 font-medium"><i class="fas fa-copy mr-1"></i>Copy Text</button>
              <button onclick="downloadRTIPDF()" class="text-xs bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 font-medium"><i class="fas fa-download mr-1"></i>Download PDF</button>
            </div>
          </div>
          <div class="p-6">
        ...
        <!-- Legal References -->
        <div class="bg-navy-50 border border-navy-200 rounded-xl p-5" id="legalRefs"></div>
      </div>
    </div>
  </section>
  ...
  <script>
    async function generateRTI() {
      ...
      document.getElementById('generateBtn').innerHTML = '<span class="spinner" style="width:16px;height:16px;"></span> Generating...';
      ...
      try {
        ...
        if (json.success) {
          document.getElementById('rtiContent').textContent = json.data.content;
          document.getElementById('filingOptions').innerHTML = '<h3 class="font-bold text-sm text-ashoka-800 mb-3"><i class="fas fa-paper-plane mr-2"></i>How to File This RTI</h3>' +
            json.data.filing_options.map(o => '<div class="flex items-start gap-2 mb-2"><i class="fas fa-chevron-right text-ashoka-600 mt-0.5 text-xs"></i><div><strong class="text-sm">' + o.method + '</strong><span class="text-sm text-gray-600"> — ' + (o.url ? '<a href="' + o.url + '" target="_blank" class="text-ashoka-600 underline">' + o.url + '</a>' : o.instructions) + '</span><span class="text-xs text-gray-400 ml-2">Fee: ' + o.fee + '</span></div></div>').join('');
          document.getElementById('legalRefs').innerHTML = '<h3 class="font-bold text-sm text-navy-700 mb-3"><i class="fas fa-scale-balanced mr-2"></i>Your Legal Rights Under RTI Act 2005</h3>' +
            json.data.legal_references.map(r => '<div class="flex items-start gap-2 mb-1"><i class="fas fa-check text-navy-500 text-xs mt-1"></i><span class="text-sm text-gray-700">' + r + '</span></div>').join('');
    ...
    function copyRTI() {
      ...
      showToast('RTI text copied!', 'success');
    }
