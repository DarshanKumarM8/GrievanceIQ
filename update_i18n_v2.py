...
# Construct strings to insert
en_str = ",\n" + ",\n".join([f"        {k}: '{v}'" for k, v in en_keys.items()])
hi_str = ",\n" + ",\n".join([f"        {k}: '{v}'" for k, v in hi_keys.items()])

code = code.replace("sort_time: 'By Resolution Time'", "sort_time: 'By Resolution Time'" + en_str, 1)
code = code.replace("sort_time: 'समाधान समय के अनुसार'", "sort_time: 'समाधान समय के अनुसार'" + hi_str, 1)

for lang in ['ta', 'te', 'bn', 'mr']:
    # Add english fallback to others
    search_str = {
        'ta': "sort_time: 'தீர்வு நேரம் வாரியாக'",
        'te': "sort_time: 'పరిష్కార సమయం ప్రకారం'",
        'bn': "sort_time: 'সমাধানের সময় দ্বারা'",
        'mr': "sort_time: 'निराकरण वेळेनुसार'"
    }[lang]
